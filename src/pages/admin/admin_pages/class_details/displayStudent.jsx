...
const StudentList = ({ students, onEditStudent, onDeleteStudent }) => {
    ...
    const handleUpdateStudent = async () => {
        try {
            ...
            const response = await apiClient.put(`/student/${editingStudent.id}`, updateStudent);
        ...
        <div className="bg-white rounded shadow">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {students.map((student) => (
                            <tr key={student.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{student.phone_num}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button onClick={() => handleEditStudent(student)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                                        Edit
                                    </button>
                                    <button onClick={() => onDeleteStudent(student.id)} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 ml-2">
                                        Delete
                                    </button>
                                </td>
                            </tr>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-bold text-indigo-600">Edit Student</DialogTitle>
                    </DialogHeader>
                    <div className="mt-4">
                        ...
                        />
                        <Label htmlFor="phone_num" className="text-sm font-medium text-gray-700">
                            Phone
                        </Label>
                        <Input
                            id="phone_num"
                    ...
                    <DialogFooter className="mt-6">
                        <Button onClick={handleUpdateStudent} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-full">
                            Save Changes
                        </Button>
                    </DialogFooter>
                </DialogContent>
