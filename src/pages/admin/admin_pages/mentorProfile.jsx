...
import { User, Phone, Home, School, BookOpen, Mail, MapPin, AlertCircle } from 'lucide-react';
...
const MentorProfile = () => {
    ...
    const fetchSchoolsAndClassesData = async (schools) => {
        const schoolPromises = schools.map(school => getSchoolData(school.school_id));
        ...
        setSchoolsData(Object.fromEntries(schoolsData.map(school => [school.id, school])));
        setClassesData(Object.fromEntries(classesData.map(classData => [classData.id, classData])));
    };
        ...
        <div className="flex h-screen bg-gray-100">
            ...
            <div className="flex-1 overflow-auto">
                <main className="p-6 ml-56">
                    <h1 className="text-4xl font-bold mb-8 text-gray-800">Mentor Profile</h1>
                                        ...
                                        <div key={school.school_id} className="mb-6">
                                            <div className="bg-blue-50 p-4 rounded-lg shadow-sm mb-4">
                                                <h3 className="text-xl font-semibold text-blue-600 mb-2">
                                                    {schoolsData[school.school_id]?.school_name || `School ID: ${school.school_id}`}
                                                </h3>
                                            ...
                                            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
                                                <h4 className="text-lg font-semibold mb-2 flex items-center">
                                                    <BookOpen className="mr-2 w-5 h-5 text-gray-600" /> Classes
                                                </h4>
                                                <div className="flex flex-wrap gap-2">
                                                    {school.classes.map((classId) => (
                                                        <span key={classId} className="px-2 py-1 bg-gray-200 rounded-full text-sm">
                                                            {classesData[classId]?.class_name || `Class ${classId}`}
                                                        </span>
                                                    ))}
                                                </div>
                                            </div>
                                            {index < teacher.schools.length - 1 && <hr className="my-4 border-t-2 border-gray-300" />}
                                        </div>
                                    ))
                                ) : (
                                    <div className="flex flex-col items-center justify-center h-full">
                                        <AlertCircle className="w-12 h-12 text-gray-400 mb-4" />
                                        <p className="text-lg text-gray-600">No schools and classes were assigned to you.</p>
                                    </div>
                                )}
                            </CardContent>
                        </Card>
